...
class Product(BaseModel):
    ...
    class Meta:
        ...
        indexes = [
            # ----------------------------------------------------------
            # Частичные (conditional) индексы — только для ACTIVE товаров.
            #
            # Зачем: 95% запросов фильтруют status='active'.
            # Индекс по всем статусам — трата места на DRAFT/ARCHIVED.
            # Partial index содержит только ACTIVE-строки → меньше размер,
            # быстрее обновление.
            #
            # condition=models.Q(status=ProductStatus.ACTIVE) —
            #   PostgreSQL WHERE-условие на индекс.
            # ----------------------------------------------------------

            # Фильтр «активные товары в категории X»
            models.Index(
                fields=['status', 'primary_category'],
                condition=models.Q(status=ProductStatus.ACTIVE),
                name='product_active_category_idx',
            ),
            # Фильтр «активные товары бренда X»
            models.Index(
                fields=['status', 'brand'],
                condition=models.Q(status=ProductStatus.ACTIVE),
                name='product_active_brand_idx',
            ),
            # Сортировка «топ по рейтингу»
            models.Index(
                fields=['-rating'],
                condition=models.Q(status=ProductStatus.ACTIVE),
            ...
            # «Рекомендуемые» — featured flag + active
            models.Index(
                fields=['-created_at'],
                condition=models.Q(
            ...
            # Оптимизирован для «много элементов, мало изменений».
            # Для tsvector — идеальный: вектор состоит из десятков лексем,
            # обновляется редко, читается часто.
            # ----------------------------------------------------------
            GinIndex(
