...
class ProductQuerySet(models.QuerySet):
    ...
    def with_variants(self):
        ...
        variants_qs = (
            ...
            # select_related для price и stock — JOIN в том же запросе.
            .select_related('price', 'stock')
            # prefetch_related для EAV атрибутов (VariantAttribute → Attribute + AttributeValue)
            # Без этого каждый variant.attributes.all() → N+1 запрос.
